...
**⚠️ MANDATORY: Trigger-Action Rules**

| Trigger | Action |
| :--- | :--- |
| **Modify/Write PRD** | MUST read `.specgraph/design/des_prompt_prd_writer.md` FIRST. |
| **Create Feature** | MUST read `substrate/sub_meta_schema.yaml` AND `design/des_architecture.yaml` (Granularity). |
| **Create Component** | MUST read `design/des_architecture.yaml` AND check parent `feat_{name}.yaml`. |
