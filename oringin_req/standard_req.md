...
| **Day 2-3** | Phase 1 | AI完善CLI，实现 `init` 和 `scan` | 为DevSpec源码生成 `.specgraph` |
...
```
DevSpec
├── M1: 知识图谱引擎 (specgraph)     ← P0 必须 (含全景视图)
...
1.  **产品根节点 (Root)**：
    *   隐式存在，由 `.specgraph/product.yaml` 定义。
...
# Step 2: 补充L1/L2定义
$ vim .specgraph/features/feat_payment.yaml
...
## 10. 需求变更流程 (Change Request Protocol)

当用户提出新需求或变更请求时，必须遵循以下流程：
...
└─────────────────────────────────────┘
    ...
    ▼
┌─────────────────────────────────────┐
│  Step 1: 文档更新 (Document First)   │
│  - 如果 PRD 未涵盖 → 更新 PRD        │
│  - 如果涉及新 Domain → 更新 product  │
└─────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────┐
│  Step 2: Feature 定义 (L1)           │
│  - 创建 feat_xxx.yaml               │
...
| 检查点 | 问题 | 不通过则 |
|--------|------|----------|
| PRD Check | 需求是否在 PRD 范围内？ | 先更新 PRD |
| Domain Check | 是否需要新 Domain？ | 先更新 product.yaml |
| Feature Check | Feature YAML 是否存在？ | 先创建 Feature |
| Component Check | Component 设计是否完整？ | 先完成设计，禁止编码 |
...
1. 首先检查 Feature.realized_by 字段
2. 如果为空或指向的 Component 不存在：
   → 禁止编写 Python 代码
   → 先生成 Component YAML 草稿
   → 请求人类确认设计
3. 只有 L2 设计完成后，才能进入 L3 编码
```
...
# 2. 创建基础结构
mkdir -p devspec .specgraph/features .devspec
...
# .specgraph/features/feat_specgraph.yaml

# 5. 验证：运行 bootstrap.py 生成 Prompt
python bootstrap.py --feature feat_specgraph
