...
> **Version**: 0.4.0 (Architectural Alignment)
...
### 1.1 The Ouroboros Vision (Meta-Goal)
...
### 1.2 Explicit Definition (Physical Reality)
...
1.  **Bilingual Structure (双语结构)**: 标题/术语使用英文，描述/逻辑使用中文。
2.  **Anchor Injection (锚点注入)**: 关键节点必须注入 `<!-- id: <node_id> -->` 以实现自动化追踪。
3.  **Single Source of Truth (唯一事实来源)**: 当 PRD 与 YAML 冲突时，以 PRD 的意图为准。

### 2.3 Interaction Principles <!-- id: des_interaction -->

*   **Structured Dialogue (结构化对话)**: 对话是状态机的流转。AI 必须严格遵守 "Receive -> Confirm Understanding -> Plan -> Confirm Plan -> Execute" 的交互协议。
*   **Context Minimalism (上下文极简)**: 针对人类有限的认知带宽，AI 提供的输出必须经过降噪，只展示当前关注点相关的信息。
...
**Abstraction Levels (抽象层级)**:

| Level | Concept | Definition |
| :--- | :--- | :--- |
| L0 | Domain | 战略版图 (Strategic Scope) |
| L1 | Feature | 用户价值单元 (User Value Unit) |
| L2 | Component | 代码详细设计 (Detailed Design) |
| L3 | Code Symbol | 物理代码投影 (Physical Projection) |

**Component Granularity Rule (L2 粒度原则)**:

*   **设计即代码**: Component 必须包含 API 签名、伪代码逻辑、关键常量。目标是 AI 可据此还原 90-95% 的代码。
*   **物理绑定**: 必须通过 `file_path` 关联到物理文件。
...
| `design.logic` | ✅ 必填 | 伪代码逻辑：自然语言+编号步骤。 |
| `design.constants` | ⚠️ 条件 | 影响输出的关键常量/模板。 |
| `design.output_files` | ⚠️ 条件 | 生成文件的路径和格式定义。 |

**YAML Schema Reference**:

所有 YAML 文件的完整格式定义位于 `.specgraph/substrate/sub_meta_schema.yaml`。

| 节点类型 | 路径模式 | 说明 |
| :--- | :--- | :--- |
| **Product** | `.specgraph/product.yaml` | 产品根节点 |
| **Feature** | `.specgraph/features/feat_{name}.yaml` | 功能节点 |
| **Component** | `.specgraph/components/comp_{name}.yaml` | 组件节点 |
| **Design** | `.specgraph/design/des_{name}.yaml` | 设计节点 |
| **Substrate** | `.specgraph/substrate/sub_{name}.yaml` | 基质节点 |

**ID Assignment Principle (ID 分配原则)**:

只有满足以下条件之一的内容才需要独立 ID：

| 条件 | 说明 |
| :--- | :--- |
| **可被引用** | 其他节点需要通过 `depends_on`、`realized_by` 等关系引用它 |
| **独立验收** | 可作为独立单元被验收或追踪 |
| **独立演进** | 有自己的生命周期，可能独立于父节点变更 |

### 3.3 Technical Stack Constraints <!-- id: sub_tech_stack -->

*   **Language**: Python 3.10+ (强制 Type Hints)
*   **Env Management**: `uv`
*   **CLI Framework**: `Typer`
*   **UI/UX**: `Rich` (终端可视化)
*   **Core Parsing**: `Tree-sitter`
*   **Storage**: `PyYAML` (Source) + `SQLite` (Runtime Cache)

---

## 4. Domain: Core Engine (`dom_core`) <!-- id: dom_core -->

**Description**: 系统的"大脑"，负责维护知识图谱、解析代码、管理上下文。

### 4.1 Feature: Meta-Schema Management <!-- id: sub_meta_schema -->

定义 Feature/Component 的 YAML 结构规范与粒度标准，防止图谱熵增。

### 4.2 Feature: Consistency Monitor <!-- id: feat_consistency_monitor -->
...
### 4.3 Feature: SpecGraph Engine <!-- id: feat_specgraph_engine -->
...
### 4.4 Feature: SpecGraph Database <!-- id: feat_specgraph_database -->
...
### 4.5 Feature: Code Scanner <!-- id: feat_code_scanner -->
...
## 5. Domain: CLI Interface (`dom_cli`) <!-- id: dom_cli -->

**Description**: 系统的"AI 接口"，为 AI Agent (Claude Code, Gemini CLI) 提供命令行工具，支持 AI 开发产品需求。

### 5.1 Feature: Command Structure <!-- id: feat_cli_command_structure -->
...
## 6. Domain: Quality Assurance (`dom_quality`) <!-- id: dom_quality -->

**Description**: 系统的"免疫系统"，负责确保代码与 Spec 的一致性。

### 6.1 Feature: PRD Format Validator <!-- id: feat_quality_prd_validator -->
...
## 7. Domain: Infrastructure (`dom_infra`) <!-- id: dom_infra -->
...
### 7.1 Feature: Logging <!-- id: feat_logging -->
...
### 7.2 Feature: Config Management <!-- id: feat_config_management -->
...
### 7.3 Feature: Error Handling <!-- id: feat_error_handling -->
...
## 8. Domain: SpecGraph Viewer (`dom_specview`) <!-- id: dom_specview -->
...
### 8.1 Feature: SpecView Dashboard Core <!-- id: feat_specview_dashboard_core -->
...
### 8.2 Feature: Hierarchy View <!-- id: feat_specview_hierarchy_view -->
...
### 8.3 Feature: Design View <!-- id: feat_specview_design_view -->
...
### 8.4 Feature: Relation View <!-- id: feat_specview_relation_view -->
...
### 8.5 Feature: Search <!-- id: feat_specview_search -->
...
## 9. Domain: Frontend Infrastructure (`dom_frontend`) <!-- id: dom_frontend -->
...
### 9.1 Feature: Component Library <!-- id: feat_frontend_component_library -->
